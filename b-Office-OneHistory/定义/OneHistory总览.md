...
本项目。定位是 **AI 侧的常驻工作区与模块开发管线**，与 Mercury 的定位对称：

- **Mercury 是人的翻译官**：活动坞、全局快捷键、命令工作台，让人快速驱动系统。
- **Diana 是 AI 的翻译官**：公共文档区、隔离 Git 工作树、候选构建与调用型试用、
  主树正式发布、项目巡检、MCP 工具中继和 z 文档通道，让 AI 按统一合同开发整个模块体系。

Diana 不进入用户的日常界面，`ui: false`。模块开发的唯一流程入口是
`b-Office-Diana/package/模块开发手册.md`；MCP 命令分为 readonly 与 standard，候选模块命令
不直接进入正式工具表。
